...
export function mergeShipSnapshots(previousSnapshot, nextSnapshot) {
  ...
  const ships = nextSnapshot.ships.map((ship) => {
    const previousShip = previousShips.get(shipKey(ship));
    if (!previousShip) {
      return ship;
    }
    const carryPreviousPosition = shouldCarryPreviousPosition(previousShip, ship);
    ...
    return {
      ...
      latitude: carryPreviousPosition ? previousShip.latitude : ship.latitude ?? null,
      longitude: carryPreviousPosition ? previousShip.longitude : ship.longitude ?? null,
      course: carryPreviousPosition ? previousShip.course : ship.course ?? null,
      heading: carryPreviousPosition ? previousShip.heading : ship.heading ?? null,
      speedKnots: carryPreviousPosition ? previousShip.speedKnots : ship.speedKnots ?? null,
      ...
      activeSailing: ship.activeSailing || previousShip.activeSailing || null
    };
  });
