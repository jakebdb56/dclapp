...
import { FALLBACK_DISNEY_PORTS, collectDisneyPorts, withKnownPortDetailUrls } from "./disney-ports.js";
import {
  getCacheDebugInfo,
  isCacheConfigured,
  readCachedSnapshot,
...
function addCacheMetadata(snapshot, cacheStatus) {
  return {
    ...snapshot,
    ports: snapshot.ports?.length ? withKnownPortDetailUrls(snapshot.ports) : FALLBACK_DISNEY_PORTS,
    connection: {
      ...snapshot.connection,
      cacheStatus
    }
