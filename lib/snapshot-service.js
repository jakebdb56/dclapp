...
import { FALLBACK_DISNEY_PORTS, collectDisneyPorts, mergePorts, withKnownPortDetailUrls } from "./disney-ports.js";
import {
  getCacheDebugInfo,
  isCacheConfigured,
  readCachedSnapshot,
...
import { attachActiveSailingsToSnapshot, collectOfficialSailings } from "./disney-sailings.js";
...
function addCacheMetadata(snapshot, cacheStatus) {
  const ports = snapshot.ports?.length
    ? mergePorts([FALLBACK_DISNEY_PORTS, snapshot.ports])
    : FALLBACK_DISNEY_PORTS;

  return {
    ...snapshot,
    ports: withKnownPortDetailUrls(ports),
    connection: {
      ...snapshot.connection,
      cacheStatus
    }
...
export async function refreshAndCacheSnapshot({
  ...
  const sailings = await collectOfficialSailings({
    timeoutMs: Math.min(timeoutMs + 3000, 12000)
  }).catch(() => []);
  let merged = null;
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    const nextWithSailings = attachActiveSailingsToSnapshot(next, sailings, referenceDate);

    const nextWithCachedPositions =
      cached && hasUsefulShipData(nextWithSailings)
        ? mergeShipSnapshots(cached, nextWithSailings)
        : nextWithSailings;
